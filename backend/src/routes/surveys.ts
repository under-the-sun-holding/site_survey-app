 ...
 */
function extractCoords(body: SurveyInput): { lon: number; lat: number; accuracy?: number } | null {
  if (body.location?.type === 'Point' && Array.isArray(body.location.coordinates)) {
    const [lon, lat] = body.location.coordinates;
    return { lon, lat };
  }
  if (body.latitude != null && body.longitude != null) {
    return { lon: body.longitude, lat: body.latitude, accuracy: body.gps_accuracy };
  }
  return null;
}
    ...
    for (const { action, survey } of surveys) {
      try {
        ...
        if (action === 'create') {
          ...
          const surveyId: string = (survey.id as string) || (idRows[0].id as string);
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (body.checklist?.length) await upsertChecklist(client, newId, body.checklist);
...
router.post('/:id/photos', upload.array('photos', 20), async (req: Request, res: Response) => {
  ...
  const { rows } = await pool.query('SELECT id FROM surveys WHERE id = $1', [id]);
  if (rows.length === 0) {
    res.status(404).json({ error: 'Survey not found' });
    return;
  }
